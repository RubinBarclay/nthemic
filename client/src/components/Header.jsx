...
const Header = ({ playlist, play }) => {
  ...
  return (
    ...
    <div className="relative flex items-center overflow-hidden h-1/2">
      <img className="absolute z-0 w-full pointer-events-none filter blur brightness-90" src={playlist.albumCoverLG} alt="background image"/>
      <img className="z-10 ml-40 mr-8 bg-red-200 w-80 h-80" src={playlist.albumCoverLG} alt="album cover"/> 
      <div className="z-10 text-white">
        {/*<h2 className="text-5xl">{playlist.name}</h2> */}
        <h3 className="w-2/3 my-2 text-5xl">{playlist.description}</h3>
